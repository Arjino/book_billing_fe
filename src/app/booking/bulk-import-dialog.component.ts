...
export class BulkImportDialogComponent implements OnInit {
  ...
  constructor(public dialogRef: MatDialogRef<BulkImportDialogComponent>) {}
  ...
  validateAndTransformData(data: any[]): void {
    ...
    data.forEach((row, index) => {
      ...
      // Validate HSN
      const hsn = Number(row.hsn);
      if (!row.hsn || isNaN(hsn) || hsn < 100000 || hsn > 9999999999) {
      ...
      if (errors.length > 0) {
        ...
        // Add valid book to extraction list
        const book: Book = {
