...
export class PurchaseReturnsComponent implements OnInit {
  ...
  private readonly pageSizeStorageKey = 'purchaseReturns.pageSize';
  ...
  loadPurchaseReturns(): void {
    this.loading = true;
    this.errorMessage = '';
    this.loadingService.show('Loading purchase returns...');

    this.purchaseService.getPurchaseReturns({
      ...
      next: (page) => {
        this.purchaseReturns = page.content || [];
      ...
      error: async (error) => {
        ...
        this.loading = false;
        this.loadingService.hide();
        this.errorMessage = await extractHttpErrorMessage(error, 'Failed to load purchase returns.');
  ...
  getPartyLabel(row: PurchaseReturn): string {
    return row.party?.name || row.partyName || String(row.partyId || '-');
  }

  getItemsCount(row: PurchaseReturn): number {
    return row.items?.length || 0;
  }

  getTotalQuantity(row: PurchaseReturn): number {
    return (row.items || []).reduce((total, item) => total + Number(item.qty || 0), 0);
  }

  getEmptyStateMessage(): string {
    if (this.startDate || this.endDate) {
      return 'No purchase returns found for the selected date filters.';
    }
